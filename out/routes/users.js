...
// OAuth login handler for frontend
const oauthLogin = (ctx, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const result = yield model.findByUsername(username);
        if (result.length > 0) {
            const user = result[0];
            const { id, username: uname, email, avatarurl, about, role } = user;
            const links = {
                self: `http://${ctx.host}${prefix}/${id}`,
            };
            ctx.body = { id, username: uname, email, about, avatarurl, role, links };
        }
        else {
            ctx.status = 401;
...
router.post('/oauth/login', (0, koa_bodyparser_1.default)(), oauthLogin);
// Add Google OAuth routes
const authRouter = new koa_router_1.default({ prefix: '/api/v1/auth' });
exports.authRouter = authRouter;
authRouter.get('/google', auth_1.googleAuth);
