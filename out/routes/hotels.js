...
const router = new koa_router_1.default({ prefix: "/api/v1/hotels" });
...
const getById = (ctx, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let hotel = yield model.getById(id); // Changed from 'article' to 'hotel'
    if (hotel.length) {
        ctx.body = hotel[0];
        ctx.status = 200;
    }
...
const deleteHotel = (ctx, next) => __awaiter(void 0, void 0, void 0, function* () {
    let id = +ctx.params.id;
    if (ctx.state.user.user.role === "admin") {
        let hotel = yield model.deleteById(id); // Changed from 'article' to 'hotel'
        ctx.status = 201;
...
router.get("/", getAll);
router.post("/", auth_1.basicAuth, (0, koa_bodyparser_1.default)(), validation_1.validateHotel, createHotel); // Changed from createArticle
router.get("/:id([0-9]{1,})", getById);
router.put("/:id([0-9]{1,})", auth_1.basicAuth, (0, koa_bodyparser_1.default)(), validation_1.validateHotel, updateHotel); // Changed from updateArticle
router.delete("/:id([0-9]{1,})", auth_1.basicAuth, deleteHotel); // Changed from deleteArticle
