...
export const getAll = async  (limit=10, page=1, order:any, direction:any) =>{
  ...
  const query = "SELECT * FROM hotel LIMIT  ? OFFSET  ?;";
  const data = await db.run_query(query, [limit, offset]);
  return data;
}

export const add = async(hotel: any) => {
  ...
  let query = `INSERT INTO hotel (${key}) VALUES (${param})`;
  try {
...
export const  update= async(hotel:any,id:any)  =>{  
 ...
 // console.log("updateString ", updateString)
  let query = `UPDATE hotel SET ${updateString} WHERE ID=${id} RETURNING *;`
  try{
...
export const deleteById = async (id:any) => {
  let query = "Delete FROM hotel WHERE ID = ?"
  let values = [id]
  try{
    await db.run_query(query, values);  
