import { basicAuth, googleAuth, googleCallback } from '../controllers/auth';
...
const oauthLogin = async(ctx: any, next: any) => {
  ...
  try {
    ...
    const result = await model.findByUsername(username) as User[];
    if (result.length > 0) {
...
router.post('/oauth/login', bodyParser(), oauthLogin);
